...
Initial support for the bookmark/outline of the PDF file has already been available ([mdbook-pdf-outline](https://pypi.org/project/mdbook-pdf-outline/)). It is written in Python and is another backend for `mdbook` and should be used with `mdbook-pdf`.
...
You can find the outlined version at `book/pdf-outline/output.pdf`. Your PDF file now have the same outline/bookmark as the one generated by `wkhtmltopdf`.
